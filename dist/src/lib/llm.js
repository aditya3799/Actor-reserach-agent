 ...
 */
export async function planSubQuestions(query) {
 ...
 */
export async function synthesizeAnswer(subQuestion, sources) {
    const openai = getGroqClient();
    if (openai) {
        try {
 ...
 */
export async function compileReport(query, findings) {
    const openai = getGroqClient();
    if (openai) {
        try {
