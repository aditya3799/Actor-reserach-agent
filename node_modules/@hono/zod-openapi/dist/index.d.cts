...
type InputTypeBase<R extends RouteConfig, Part extends string, Type extends keyof ValidationTargets> = R["request"] extends RequestTypes ? RequestPart<R, Part> extends ZodType ? {
  in: { [K in Type]: HasUndefined<ValidationTargets[K]> extends true ? { [K2 in keyof z.input<RequestPart<R, Part>>]?: z.input<RequestPart<R, Part>>[K2]; } : { [K2 in keyof z.input<RequestPart<R, Part>>]: z.input<RequestPart<R, Part>>[K2]; }; };
...
type RouteConfigToTypedResponse<R extends RouteConfig> = { [Status in DefinedStatusCodes<R>]: R["responses"][Status] extends {
  content: infer Content;
} ? undefined extends Content ? never : ReturnJsonOrTextOrResponse<keyof R["responses"][Status]["content"], ExtractContent<R["responses"][Status]["content"]>, Status> : TypedResponse<{}, ExtractStatusCode<Status>, string>; }[DefinedStatusCodes<R>] | ("default" extends keyof R["responses"] ? R["responses"]["default"] extends {
  content: infer Content;
...
type HandlerFromRoute<R extends RouteConfig, E extends Env> = Handler<E, ConvertPathType<R["path"]>, ComputeInput<R>, R extends {
  responses: {
    [statusCode: number]: {
      content: {
        [mediaType: string]: ZodMediaTypeObject;
...
type HookFromRoute<R extends RouteConfig, E extends Env> = Hook<ComputeInput<R>, E, ConvertPathType<R["path"]>, R extends {
  responses: {
    [statusCode: number]: {
      content: {
        [mediaType: string]: ZodMediaTypeObject;
...
} ? MaybePromise<RouteConfigToTypedResponse<R>> | undefined : MaybePromise<RouteConfigToTypedResponse<R>> | MaybePromise<Response> | undefined> | undefined;
