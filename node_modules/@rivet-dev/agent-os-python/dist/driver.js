...
import { TIMEOUT_ERROR_MESSAGE, TIMEOUT_EXIT_CODE, createFsStub, createNetworkStub, filterEnv, wrapFileSystem, wrapNetworkAdapter, } from "@secure-exec/core";
const PYTHON_PACKAGE_UNSUPPORTED_ERROR = "ERR_PYTHON_PACKAGE_INSTALL_UNSUPPORTED: Python package installation is not supported in this runtime";
const PACKAGE_INSTALL_PATHWAYS_PATTERN = /\b(micropip|loadPackagesFromImports|loadPackage)\b/;
const MAX_SERIALIZED_VALUE_BYTES = 4 * 1024 * 1024;
function normalizeCpuTimeLimitMs(timeoutMs) {
...
function ensurePackageInstallPathwaysAreDisabled(code) {
...
`;
export class PyodideRuntimeDriver {
    options;
    worker = null;
    pending = new Map();
    defaultOnStdio;
        ...
        try {
            switch (message.method) {
                ...
                case "networkFetch": {
                    ...
                    const options = typeof message.params.options === "object" && message.params.options !== null
    ...
    async ensureWorkerReady() {
        ...
        this.readyPromise = this.callWorker("init", {
    ...
    async restartWorkerAfterTimeout() {
    ...
    async runWithTimeout(requestFactory, timeoutMs) {
        if (timeoutMs === undefined) {
            return {
                timedOut: false,
        ...
        return new Promise((resolve, reject) => {
            ...
            void requestFactory().then((value) => {
                if (settled) {
                    return;
                }
                settled = true;
    ...
    async run(code, options = {}) {
        try {
            ...
            const timeoutMs = normalizeCpuTimeLimitMs(options.cpuTimeLimitMs ?? this.defaultCpuTimeLimitMs);
            const hook = options.onStdio ?? this.defaultOnStdio;
            const envOverrides = options.env === undefined
                ? undefined
                : filterEnv(options.env, this.options.system.permissions);
            const result = await this.runWithTimeout(() => this.callWorker("run", {
                code,
                options: {
    ...
    async exec(code, options) {
        try {
            ...
            const envOverrides = options?.env === undefined
                ? undefined
                : filterEnv(options.env, this.options.system.permissions);
            const result = await this.runWithTimeout(() => this.callWorker("exec", {
                code,
                options: {
                    cwd: options?.cwd,
                    env: envOverrides,
