 ...
 */
import { FILETYPE_CHARACTER_DEVICE, FILETYPE_DIRECTORY, FILETYPE_REGULAR_FILE, RIGHTS_STDIO, RIGHTS_FILE_ALL, RIGHTS_DIR_ALL, ERRNO_SUCCESS, ERRNO_EBADF, } from './wasi-constants.js';
import { FDEntry, FileDescription, } from './wasi-types.js';
// ---------------------------------------------------------------------------
// FDTable
// ---------------------------------------------------------------------------
export class FDTable {
    ...
    constructor() {
        ...
        this._fds.set(0, new FDEntry({ type: 'stdio', name: 'stdin' }, FILETYPE_CHARACTER_DEVICE, RIGHTS_STDIO, 0n, 0));
        this._fds.set(1, new FDEntry({ type: 'stdio', name: 'stdout' }, FILETYPE_CHARACTER_DEVICE, RIGHTS_STDIO, 0n, 0));
        this._fds.set(2, new FDEntry({ type: 'stdio', name: 'stderr' }, FILETYPE_CHARACTER_DEVICE, RIGHTS_STDIO, 0n, 0));
    }
    /**
    ...
    open(resource, options = {}) {
        const { filetype = FILETYPE_REGULAR_FILE, rightsBase = (filetype === FILETYPE_DIRECTORY ? RIGHTS_DIR_ALL : RIGHTS_FILE_ALL), rightsInheriting = (filetype === FILETYPE_DIRECTORY ? RIGHTS_FILE_ALL : 0n), fdflags = 0, path, } = options;
        const inode = resource.ino ?? 0;
    ...
    dupMinFd(fd, minFd) {
        ...
        entry.fileDescription.refCount++;
        let newFd = minFd;
        while (this._fds.has(newFd))
            newFd++;
        this._fds.set(newFd, new FDEntry(entry.resource, entry.filetype, entry.rightsBase, entry.rightsInheriting, entry.fdflags, entry.path ?? undefined, entry.fileDescription));
...
//# sourceMappingURL=fd-table.js.map