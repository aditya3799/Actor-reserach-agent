...
// Test suite
// ---------------------------------------------------------------------------
export function defineFsDriverTests(config) {
    ...
    describe(name, () => {
        ...
        describe("core", () => {
            ...
            test.skipIf(!capabilities.removeDir)("rename a directory with children moves all children", async () => {
        ...
        describe.skipIf(!capabilities.symlinks)("symlinks", () => {
            ...
            test("dangling symlink: lstat succeeds, stat/readFile throws ENOENT", async () => {
                ...
                const lstatResult = await fs
                    .lstat("/dangle.txt")
                    .catch((e) => e);
                if (lstatResult instanceof Error)
                    return;
                const ls = lstatResult;
        ...
        describe.skipIf(!capabilities.permissions)("permissions", () => {
            ...
            test("chown changes uid/gid", async () => {
                ...
                const result = await fs
                    ...
                    .catch((e) => e);
                if (result instanceof Error) {
                    expect(hasErrorCode(result, "EPERM") ||
                        hasErrorCode(result, "ENOSYS")).toBe(true);
                }
                else {
