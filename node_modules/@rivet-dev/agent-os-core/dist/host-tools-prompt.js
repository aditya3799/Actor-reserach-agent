import { camelToKebab, getFieldInfos, getZodEnumValues, getZodObjectShape, } from "./host-tools-argv.js";
/**
...
export function generateToolReference(toolKits) {
    ...
    lines.push("");
    lines.push("Run `node /usr/local/bin/agentos list-tools` to see all available tools.");
    lines.push("");
    for (const tk of toolKits) {
        ...
        for (const [toolName, tool] of Object.entries(tk.tools)) {
            ...
            lines.push(`- \`node /usr/local/bin/agentos-${tk.name} ${toolName}${flagStr}\` — ${tool.description}`);
        }
        lines.push("");
        // Include examples if any tool has them
        const toolsWithExamples = Object.entries(tk.tools).filter(([, t]) => t.examples && t.examples.length > 0);
        if (toolsWithExamples.length > 0) {
            ...
            for (const [toolName, tool] of toolsWithExamples) {
                for (const ex of tool.examples ?? []) {
                    ...
                    lines.push(`- ${ex.description}: \`node /usr/local/bin/agentos-${tk.name} ${toolName}${flagArgs ? ` ${flagArgs}` : ""}\``);
                }
            }
            lines.push("");
        }
        lines.push(`Run \`node /usr/local/bin/agentos-${tk.name} <tool> --help\` for details.`);
        lines.push("");
    }
    return lines.join("\n");
}
 ...
 */
function buildFlagSignature(schema) {
