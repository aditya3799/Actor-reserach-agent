import { deserializeMessage, isRequest, isResponse, serializeMessage, } from "./protocol.js";
...
export class AcpClient {
    ...
    _nextId = 1;
    _pending = new Map();
    _seenInboundRequestIds = new Set();
    _notificationHandlers = [];
    _closed = false;
    _timeoutMs;
    _stdoutIterator = null;
    _readerClosed = false;
    _pendingPermissionRequests = new Map();
    _requestHandler;
    _recentActivity = [];
    ...
    request(method, params) {
        ...
        const compatibilityResponse = this._maybeHandlePermissionResponse(method, params);
    ...
    _watchExit() {
        this._process.wait().then(() => {
            this._recordActivity(`process_exit exitCode=${String(this._getProcessExitCode())} killed=${String(this._getProcessKilled())}`);
            setTimeout(() => {
                if (this._closed) {
                    return;
                }
    ...
    _handleRequest(msg) {
        ...
        if (!this._requestHandler) {
            this._process.writeStdin(serializeMessage({
                jsonrpc: "2.0",
                id: msg.id,
                error: {
                    code: -32601,
    ...
    _recordActivity(entry) {
        ...
        if (this._recentActivity.length > RECENT_ACTIVITY_LIMIT) {
            this._recentActivity.splice(0, this._recentActivity.length - RECENT_ACTIVITY_LIMIT);
        }
    }
    _createTimeoutError(method, id) {
        const processState = `process exitCode=${String(this._getProcessExitCode())} killed=${String(this._getProcessKilled())}`;
        const activity = this._recentActivity.length > 0
            ? this._recentActivity.join(" | ")
            : "no recent ACP activity";
        return new Error(`ACP request ${method} (id=${id}) timed out after ${this._timeoutMs}ms. ${processState}. Recent ACP activity: ${activity}`);
    }
    _getProcessExitCode() {
    ...
    _maybeHandlePermissionResponse(method, params) {
        if (method !== LEGACY_PERMISSION_METHOD &&
            method !== ACP_PERMISSION_METHOD) {
            return null;
        }
        const payload = this._toRecord(params);
        const permissionIdValue = payload.permissionId;
        if (typeof permissionIdValue !== "string" &&
    ...
    _normalizePermissionResult(params, pending) {
        const outcome = params.outcome;
        if (outcome && typeof outcome === "object") {
            return { outcome };
        }
        const requestedReply = params.reply;
        const selectedOptionId = this._resolvePermissionOptionId(pending.options, typeof requestedReply === "string" ? requestedReply : undefined);
        if (selectedOptionId) {
            return {
    ...
    _resolvePermissionOptionId(options, reply) {
        if (!options || !reply) {
            return null;
        }
        const targets = (() => {
    ...
    _isCancelMethodNotFound(response) {
...
function summarizeInboundMessage(msg) {
    if (isResponse(msg)) {
        if (msg.error) {
            return truncateActivityText(`received response id=${String(msg.id)} error=${msg.error.code}:${msg.error.message}`);
        }
        return `received response id=${String(msg.id)}`;
    }
    if (isRequest(msg)) {
        return truncateActivityText(`received request ${msg.method} id=${String(msg.id)}`);
    }
