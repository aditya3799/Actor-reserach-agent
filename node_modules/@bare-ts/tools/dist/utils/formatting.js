...
export function softSpace(s) {
  return s[0] === "\n" ? s : ` ${s}`;
}
export function jsDoc(content) {
  if (content === "") {
    return "";
  }
...
export function jsRpr(v) {
  return typeof v === "string" ? `"${v}"` : typeof v === "bigint" ? `${v}n` : `${v}`;
}
export function toCamelCase(s) {
  return normalize(s).toLowerCase().replace(/_./g, (m0) => m0[1].toUpperCase());
}
...
export function dent(chunks, ...values) {
