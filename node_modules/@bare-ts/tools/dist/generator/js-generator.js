...
export function generate(schema, config) {
  const g = { config, symbols: ast.symbols(schema) };
  let body = "";
...
`;
    ...
    if (!(isVoid || config.lib) && rootAliases.indexOf(aliased.alias) !== -1) {
...
`;
  ...
  if (g.config.generator !== "js") {
    head += "\n";
    const predefinedTypes = ast.NUMERIC_TAG.slice();
...
` : "";
  ...
  for (let i = 0; i < type.data.length; i++) {
    const { name } = type.data[i];
    params += `${name}_${ts ? `:${softSpace(genType(g, type.types[i]))},` : ","}
`;
...
function genSetReader(g, type) {
  const valType = type.types[0];
  const setGenerics = g.config.generator === "js" ? "" : `<${genType(g, valType)}>`;
...
export function noneVal(type) {
  return type.extra != null ? jsRpr(ast.literalVal(type.extra.literal)) : "null";
}
