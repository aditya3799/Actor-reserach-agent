...
function main() {
  try {
    ...
    if (values.help) {
      ...
      const schema = positionals.length > schemaIndex ? positionals[schemaIndex] : 0;
      const generator = values.generator;
      if (generator != null && generator !== "bare" && generator !== "dts" && generator !== "js" && generator !== "ts") {
...
function compileAction(schema, opts) {
  let config = null;
  let content = "";
  try {
    ...
    const message = `error: ${e instanceof CompilerError ? `(${formatLocation(
      config?.schema,
...
export function location(content, offset) {
