import { Logger, LevelWithSilent } from 'pino';
export { Logger } from 'pino';
import { z } from 'zod/v4';

declare const LogLevelSchema: z.ZodEnum<{
    fatal: "fatal";
    error: "error";
    warn: "warn";
    info: "info";
    debug: "debug";
    trace: "trace";
    silent: "silent";
}>;
type LogLevel = z.infer<typeof LogLevelSchema>;
declare function getPinoLevel(logLevel?: LogLevel): LevelWithSilent;
declare function getIncludeTarget(): boolean;
/**
 * Configure a custom base logger.
 */
declare function configureBaseLogger(logger: Logger): void;
/**
 * Create the default logger with optional log level.
 */
declare function makeDefaultLogger(logLevel?: LogLevel): Logger;
/**
 * Configure the default logger with optional log level.
 */
declare function configureDefaultLogger(logLevel?: LogLevel): void;
/**
 * Get or initialize the base logger.
 */
declare function getBaseLogger(): Logger;
/**
 * Returns a child logger with `target` bound for the given name.
 */
declare function getLogger(name?: string): Logger;

export { type LogLevel, LogLevelSchema, configureBaseLogger, configureDefaultLogger, getBaseLogger, getIncludeTarget, getLogger, getPinoLevel, makeDefaultLogger };
