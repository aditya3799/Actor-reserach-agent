...
function errorMessage(error, fallback = String(error)) {
...
function isActorAbortedError(error) {
  return isRivetErrorLike(error) && error.group === "actor" && error.code === "aborted";
}
...
var RivetError = (_class = class extends Error {
  ...
  constructor(group, code, message, options) {
    ...
    super(message, { cause: normalized.cause });_class.prototype.__init.call(this);;
...
var UserError = class extends RivetError {
  constructor(message, options) {
    super("user", _nullishCoalesce((options == null ? void 0 : options.code), () => ( USER_ERROR_CODE)), message, {
      public: true,
...
function toRivetError(error, fallback) {
  ...
  return new RivetError(
    _nullishCoalesce((fallback == null ? void 0 : fallback.group), () => ( "actor")),
    _nullishCoalesce((fallback == null ? void 0 : fallback.code), () => ( INTERNAL_ERROR_CODE)),
    errorMessage(error, _nullishCoalesce((fallback == null ? void 0 : fallback.message), () => ( "Unknown error"))),
    {
...
function encodeBridgeRivetError(error) {
...
function internalError(message, options) {
  return new RivetError(
    _nullishCoalesce((options == null ? void 0 : options.group), () => ( "actor")),
    _nullishCoalesce((options == null ? void 0 : options.code), () => ( INTERNAL_ERROR_CODE)),
    message,
    {
...
function invalidEncoding(format) {
  return new RivetError(
    "encoding",
    "invalid",
...
function actorStopping(identifier) {
  return new RivetError(
    "actor",
    "stopping",
...
function actorRestarting(opts) {
  return new RivetError(
    ...
    "Actor is restarting. Retry the request.",
    {
      ...
      metadata: {
        retryable: true,
        ...(opts == null ? void 0 : opts.phase) ? { phase: opts.phase } : {},
        ...(opts == null ? void 0 : opts.retryAfterMs) !== void 0 ? { retryAfterMs: opts.retryAfterMs } : {}
      }
    }
  );
}
