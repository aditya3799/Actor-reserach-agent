...
var ToClientBodySchema = z.discriminatedUnion("tag", [
...
var ToClientSchema = z.object({
  body: ToClientBodySchema
});
var ActionRequestSchema = z.object({
  id: UintSchema,
  name: z.string(),
  args: z.unknown()
});
var SubscriptionRequestSchema = z.object({
  eventName: z.string(),
  subscribe: z.boolean()
});
var ToServerBodySchema = z.discriminatedUnion("tag", [
...
var ToServerSchema = z.object({
  body: ToServerBodySchema
});
var HttpActionRequestSchema = z.object({
  args: z.unknown()
});
var HttpActionResponseSchema = z.object({
  output: z.unknown()
});
var HttpQueueSendRequestSchema = z.object({
  body: z.unknown(),
  name: z.string().optional(),
  wait: z.boolean().optional(),
  timeout: z.number().optional()
});
var HttpQueueSendResponseSchema = z.object({
  status: z.enum(["completed", "timedOut"]),
  response: z.unknown().optional()
});
var HttpResponseErrorSchema = z.object({
  group: z.string(),
  code: z.string(),
  message: z.string(),
  metadata: z.unknown().optional(),
...
function resolveActorGatewayOptions(defaults = {}, overrides) {
  const skipReadyWait = (overrides == null ? void 0 : overrides.skipReadyWait) ?? defaults.skipReadyWait ?? false;
  return {
    skipReadyWait
  };
}
...
function buildLifecycleBoundaryInfo(kind, source, message, opts) {
  return {
    ...
    message,
    legacy: (opts == null ? void 0 : opts.legacy) ?? false
  };
}
...
async function waitWithSignal(ms, signal) {
  ...
  await new Promise((resolve, reject) => {
    ...
    const onAbort = () => {
      ...
      cleanup();
      reject((signal == null ? void 0 : signal.reason) ?? new Error("Operation aborted"));
    };
    const cleanup = () => {
...
async function retryOnLifecycleBoundary(run, opts) {
  const maxAttempts = (opts == null ? void 0 : opts.maxAttempts) ?? 5;
  const initialDelayMs = (opts == null ? void 0 : opts.initialDelayMs) ?? 25;
  const maxDelayMs = (opts == null ? void 0 : opts.maxDelayMs) ?? 200;
  let lastError;
...
async function sendHttpRequest(opts) {
  ...
  if (opts.method === "POST" || opts.method === "PUT") {
    invariant(opts.body !== void 0, "missing body");
...
var ActorConnRaw = class {
  ...
  #connectWithRetry() {
    ...
    pRetry(this.#connectAndWait.bind(this), {
      ...
      if (err instanceof AbortError || err.name === "AbortError" || !this.#shouldRetryConnectionOpenError(err)) {
  ...
  async #connectAndWait() {
    try {
      ...
      const actorError = error instanceof RivetError ? error : new RivetError(
        "client",
  ...
  async #connectWebSocket() {
    ...
    invariant2(ws, "websocket should have been created");
    logger().debug({
  ...
  async #parseMessage(data) {
    invariant2(this.#websocket, "websocket must be defined");
          ...
          return {
            body: {
              ...
              val: {
                ...
                metadata: msg.body.val.metadata ? decodeCborCompat(
                  new Uint8Array(
...
var ClientConfigSchemaBase = z2.object({
  /**
   ...
   */
  endpoint: z2.string().optional().transform((val) => {
  ...
  poolName: z2.string().default(() => getRivetPool() ?? "default"),
  encoding: EncodingSchema.default("bare"),
  headers: z2.record(z2.string(), z2.string()).optional().default(() => ({})),
  gateway: z2.object({
    skipReadyWait: z2.boolean().optional().default(false)
   ...
   */
  maxInputSize: z2.number().int().positive().default(DEFAULT_MAX_QUERY_INPUT_SIZE),
...
var BufferedRemoteWebSocket = class {
  ...
  #hasConsumer(type) {
    ...
    return Boolean(handler) || (((_a = this.#listeners.get(type)) == null ? void 0 : _a.size) ?? 0) > 0;
  }
...
function uint8ArrayToBase64Url(value) {
  return uint8ArrayToBase64(value).replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/g, "");
}
...
var RemoteEngineControlClient = class {
  ...
  async getWithKey({
    ...
    try {
      ...
      if (error instanceof RivetError && error.group === "actor" && error.code === "not_found") {
        return void 0;
      }
      throw error;
    }
