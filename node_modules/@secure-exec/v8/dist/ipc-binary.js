...
export function encodeFrame(frame) {
    ...
    if (body.length > MAX_FRAME_SIZE) {
        throw new Error(`Frame size ${body.length} exceeds maximum ${MAX_FRAME_SIZE}`);
    }
...
export function decodeFrame(buf) {
    ...
    switch (msgType) {
        ...
        default:
            throw new Error(`Unknown message type: 0x${msgType.toString(16).padStart(2, "0")}`);
    }
}
/**
...
function encodeSessionId(sid) {
    ...
    if (bytes.length > 255) {
        throw new Error(`Session ID byte length ${bytes.length} exceeds maximum 255`);
    }
...
function writeLenPrefixedU16(s) {
    const bytes = Buffer.from(s, "utf8");
    if (bytes.length > 0xffff) {
        throw new Error(`String byte length ${bytes.length} exceeds maximum 65535`);
    }
...
function readLenPrefixedU16(buf, pos) {
