...
function resolveBinaryPath() {
    ...
    const crateRelative = resolve(__dirname, "../../../native/v8-runtime/target/release/secure-exec-v8");
    if (existsSync(crateRelative))
        return crateRelative;
    const crateDebug = resolve(__dirname, "../../../native/v8-runtime/target/debug/secure-exec-v8");
    if (existsSync(crateDebug))
        return crateDebug;
...
export async function createV8Runtime(options) {
    ...
    function formatRuntimeCloseError(baseMessage) {
    ...
    child.on("exit", (code, signal) => {
        ...
        rejectPendingSessions(exitError ?? new Error("V8 runtime process exited unexpectedly"));
    });
    ...
    try {
        ...
        throw new Error(`Failed to connect to V8 runtime: ${msg}${stderrBuf ? `\nstderr: ${stderrBuf}` : ""}`);
    }
    /** Ref/unref the IPC socket based on active session count. */
    function updateSocketRef() {
        if (!ipcClient || disposed)
            return;
    ...
    function rejectPendingSessions(error) {
                        ...
                        sessionHandlers.set(sessionId, (frame) => {
                            switch (frame.type) {
                                case "BridgeCall": {
                                    ...
                                    void (async () => {
                                        try {
                                            const args = ipcDeserialize(frame.payload);
                                            const result = await handler(...(Array.isArray(args)
                                                ? args
                                                : [args]));
                                            if (!client.isConnected)
                                                return;
                                ...
                                case "StreamCallback":
                                    ...
                                    execOptions.onStreamCallback?.(frame.callbackType, frame.payload);
                                    break;
                            }
                        });
    ...
    return new Promise((resolve, reject) => {
        ...
        rl.on("close", () => {
            if (!resolved) {
                resolved = true;
                clearTimeout(timeout);
                reject(new Error("V8 runtime process closed stdout before sending socket path"));
            }
        });
        child.on("exit", (code, signal) => {
            if (!resolved) {
                resolved = true;
                clearTimeout(timeout);
                reject(new Error(`V8 runtime process exited (code=${code}, signal=${signal}) before sending socket path`));
            }
        });
    });
}
