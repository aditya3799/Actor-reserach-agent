...
class InodeMutex {
    ...
    async acquire(ino) {
        ...
        let release;
        this.locks.set(ino, new Promise((r) => {
            release = r;
        }));
        return () => {
...
function blockKey(ino, chunkIndex) {
...
function versionedBlockKey(ino, chunkIndex) {
    return `${ino}/${chunkIndex}/${randomId()}`;
}
export function createChunkedVfs(options) {
    ...
    function makeBlockKey(ino, chunkIndex) {
    ...
    function getOrCreateBuffer(ino, currentSize) {
    ...
    // -------------------------------------------------------------------
    async function resolveIno(path) {
        return metadata.resolvePath(normalizePath(path));
    }
    async function requireInode(ino) {
    ...
    async function requireFileIno(path) {
    ...
    // -------------------------------------------------------------------
    async function readInodeContent(ino, meta) {
        const buf = writeBuffers.get(ino);
        ...
        if (meta.storageMode === "inline" && buf) {
            ...
            if (meta.inlineContent)
                result.set(meta.inlineContent);
            for (const [ci, chunk] of buf.dirtyChunks) {
        ...
        for (const entry of chunkEntries) {
            if (buf?.dirtyChunks.has(entry.chunkIndex))
                continue;
    ...
    // -------------------------------------------------------------------
    async function writeInodeContent(ino, content, meta) {
        const now = Date.now();
    ...
    // -------------------------------------------------------------------
    async function promoteToChunked(ino, meta) {
    ...
    // -------------------------------------------------------------------
    async function demoteToInline(ino, content) {
        const oldKeys = await metadata.deleteAllChunks(ino);
        if (!versioning && oldKeys.length > 0) {
            await blocks.deleteMany(oldKeys);
        }
    ...
    // -------------------------------------------------------------------
    async function realpathWalk(path) {
        const parts = splitPath(normalizePath(path));
        const resolved = [];
        const inoStack = [1]; // root
        let symlinkCount = 0;
        let i = 0;
        while (i < parts.length) {
            ...
            if (childIno === null) {
                throw new KernelError("ENOENT", `no such file or directory, realpath '${path}'`);
            }
    ...
    const vfs = {
        ...
        async exists(path) {
            try {
                ...
                if (e instanceof KernelError && e.code === "ENOENT")
                    return false;
                // ELOOP on dangling/circular symlinks: treat as not existing.
                if (e instanceof KernelError && e.code === "ELOOP")
                    return false;
                throw e;
            }
        },
        ...
        async pread(path, offset, length) {
            ...
            const effectiveSize = buf ? buf.bufferedSize : meta.size;
            // Clamp.
            if (offset >= effectiveSize || length === 0)
                return new Uint8Array(0);
            ...
            try {
                ...
                if (length < currentSize) {
                    ...
                    if (length === 0) {
                        ...
                        if (!versioning && keys.length > 0)
                            await blocks.deleteMany(keys);
                        await metadata.updateInode(ino, {
                            storageMode: "inline",
                    ...
                    if (!versioning && deletedKeys.length > 0)
                        await blocks.deleteMany(deletedKeys);
                    ...
                    if (meta.storageMode === "inline") {
                        if (length <= inlineThreshold) {
                            const content = new Uint8Array(length);
                            if (meta.inlineContent)
                                content.set(meta.inlineContent);
                            await metadata.updateInode(ino, {
                                inlineContent: content,
                                size: length,
                                mtimeMs: now,
        ...
        async rename(oldPath, newPath) {
            ...
            try {
                ...
                await metadata.transaction(async () => {
                    ...
                    await metadata.renameDentry(srcResolved.parentIno, srcResolved.name, dstResolved.parentIno, dstResolved.name);
    ...
    if (writeBuffering) {
        ...
        vfs.fsync = async (_path) => { };
    }
    // Add copy method.
    vfs.copy = async (srcPath, dstPath) => {
    ...
    if (versioning && versioningStore) {
        const versioningApi = {
            ...
            async pruneVersions(path, policy) {
                const ino = await resolveIno(path);
                const release = await mutex.acquire(ino);
                try {
                    ...
                    if (policy.type === "count") {
                        ...
                        if (allVersions.length <= policy.keep)
                            return 0;
