 ...
 */
function expandStarReExports(source, hostPath) {
    ...
    const ownExportNames = new Set(ownExports
        .map((e) => e.n)
...
function buildCommonJsImportWrapper(source, filePath) {
    ...
    let namedExports = Array.from(new Set(cjsExports.filter((name) => name !== "default" &&
        name !== "__esModule" &&
...
export function transformSourceForImportSync(source, filePath, formatPath = filePath) {
    ...
    const needsTransform = processedSource.includes(UNICODE_SET_REGEX_MARKER) || syntax.hasImportMeta;
    if (!(syntax.hasModuleSyntax || syntax.hasDynamicImport || syntax.hasImportMeta)) {
