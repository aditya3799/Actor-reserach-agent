 ...
 */
export function createSandboxCommandExecutor(factory, baseSystemDriver) {
    return {
        spawn(command, args, options) {
            ...
            if (isShellCommand(command) &&
                ...
                if (parsed && isNodeCommand(parsed.command)) {
                    return spawnNodeChild(factory, baseSystemDriver, parsed.args, options);
                }
            }
            // Non-node commands not supported in standalone sandbox mode
            const err = new Error("ENOSYS: function not implemented, spawn");
            err.code = "ENOSYS";
            err.errno = -38;
            err.syscall = "spawn";
            throw err;
...
function spawnNodeChild(factory, baseSystemDriver, args, options) {
    ...
    driver = factory.createRuntimeDriver({
        ...
        runtime: {
            process: childSystemDriver.runtime?.process ?? {},
            os: childSystemDriver.runtime?.os ?? {},
        },
        onStdio: (event) => {
