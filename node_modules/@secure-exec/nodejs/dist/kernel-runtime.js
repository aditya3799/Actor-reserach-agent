...
import { allowAll, allowAllFs, createProcessScopedFileSystem, } from '@secure-exec/core';
...
export function createHostFallbackVfs(base) {
    return {
        ...
        pwrite: async (path, offset, data) => {
            try {
                return await base.pwrite(path, offset, data);
            }
            catch {
                const handle = await fsPromises.open(path, 'r+');
                try {
...
class NodeRuntimeDriver {
    ...
    _terminatingDrivers = new Map();
    _loopbackExemptPorts;
    _moduleAccessCwd;
    _packageRoots;
    constructor(options) {
    ...
    tryResolve(command) {
        ...
        if (this._resolveBinCommand(command) !== null)
            return true;
        return false;
    }
    _terminateDriver(pid, driver) {
     ...
     */
    _resolveBinCommand(command) {
        if (!this._moduleAccessCwd)
            return null;
        ...
        try {
            ...
            if (match) {
                ...
                const resolved = resolve(join(this._moduleAccessCwd, 'node_modules', '.bin'), match[1]);
                const nmDir = join(this._moduleAccessCwd, 'node_modules');
    ...
    spawn(command, args, ctx) {
        ...
        let stdinLiveSource;
        let batchStdinChunks;
        let batchStdinResolve = null;
        let batchStdinPromise;
        if (ctx.streamStdin) {
            ...
            batchStdinChunks = [];
            batchStdinPromise = new Promise((resolve) => {
        ...
        const proc = {
            ...
            writeStdin: ctx.streamStdin
                ? (data) => streamWriteStdin(data)
                : (data) => { batchStdinChunks.push(data); },
                ...
                : () => {
                    if (batchStdinResolve) {
                        if (batchStdinChunks.length === 0) {
                            ...
                            let offset = 0;
                            for (const chunk of batchStdinChunks) {
    ...
    // Async execution
    // -------------------------------------------------------------------------
    async _executeAsync(command, args, ctx, proc, resolveExit, liveStdinSource, batchStdinPromise, getKilledSignal) {
        const kernel = this._kernel;
        try {
            ...
            const effectiveStdinSource = stdinIsTTY
                ? {
                    async read() {
                        try {
            ...
            try {
                ...
                if (scriptPath.startsWith('/root/node_modules/')) {
                    ...
                    if (!hostPath && this._moduleAccessCwd) {
                        hostPath = join(this._moduleAccessCwd, 'node_modules', scriptPath.slice('/root/node_modules/'.length));
                    }
                    if (hostPath) {
                        try {
